...
do
    ...
    output1=$(./samplev1 meetingData.csv -p)
    if [ "$output" = "$output1" ]; then
        echo -e "Test $((j+i+1+20)) \033[0;32m PASS \033[0;0m csv -p largest values at  bottom, $lines lines"
    else
        echo -e "Test $((j+i+1+20)) \033[0;31m FAILED---------------\033[0;0m csv -p largest values at  bottom, $lines lines"
    fi
done
